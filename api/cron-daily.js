...
async function generateRound(category, difficulty) {
  ...
  const msg = await client.messages.create({
    ...
    messages: [{
      role: "user",
      content: `Generate 5 factual statements about "${category}" in English.${subtopicNote} EXACTLY 4 must be TRUE, EXACTLY 1 must be FALSE (subtle, realistic-sounding lie). Difficulty: ${DIFF_PROMPTS[difficulty] || "moderate"}. Never use profanity or vulgar language. Return ONLY valid JSON: {"statements":[{"text":"...","real":true},...]} No markdown, no explanation.`,
    }],
  });
