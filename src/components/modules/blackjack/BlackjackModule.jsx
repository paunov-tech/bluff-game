...
// BlackjackModule — Best-of-3 Blackjack 21 against AXIOM as a Climb warm-up.
//
...
export function BlackjackModule({ lang = "en", userId, onComplete, onAbort }) {
  ...
  function finishMatch(initialStreak) {
    ...
    // ARENA result shape — engine accumulates scoreDelta + swearDelta into
    // session totals; streakDelta seeds Climb-style streak in any later
    // module that reads incomingState.streak.
    const playerScore = session?.score?.player | 0;
    const axiomScore  = session?.score?.axiom  | 0;
    const won         = session?.matchWinner === "player";
    onComplete?.({
      success:     won,
      scoreDelta:  won ? 300 : 100,
      swearDelta:  won ? 30  : 10,
      streakDelta: initialStreak | 0,
      moduleStats: {
        playerScore, axiomScore,
        matchWinner:    session?.matchWinner || null,
        streakTransfer: initialStreak | 0,
